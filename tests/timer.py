...
for pro in p:
    for k in [15,20,25]:
        for f in files_list:
            print (pro,k,f)
            if pro=="python":
                call_l=["/usr/bin/time","-v","-o","tested/"+pro+"_tested_k_"+str(k)+f, "python3", "../python/main.py", "test_sequences/"+f, str(k)]
                call(call_l)
            elif pro=="java":
                call_l=["/usr/bin/time","-v","-o","tested/"+pro+"_tested_k_"+str(k)+f, "java", "-cp", "../java/", "Main", "main", "test_sequences/"+f, str(k)]
                call(call_l)
            elif pro=="c":
